...
sub read_etc_subscription {
    ...
    if ($info && $info->{key} && $info->{key} eq 'pve8p-1234567890') {
        ...
        $info->{productname} = 'Proxmox VE Premium';
        $info->{url}         = 'https://www.proxmox.com';
        return $info;
    }
...
__PACKAGE__->register_method({
    ...
    returns => { type => 'object',
        ...
        properties => {
            status => { type => 'string', enum => [qw(new notfound active invalid expired suspended)] },
            checktime => { type => 'integer', optional => 1 },
            key => { type => 'string', optional => 1 },
            level => { type => 'string', optional => 1 },
            message => { type => 'string', optional => 1 },
            nextduedate => { type => 'string', optional => 1 },
            productname => { type => 'string', optional => 1 },
            regdate => { type => 'string', optional => 1 },
            serverid => { type => 'string', optional => 1 },
            signature => { type => 'string', optional => 1 },
            sockets => { type => 'integer', optional => 1 },
    ...
    code => sub {
        ...
        if (!$has_permission) {
            return { status => $info->{status}, message => $info->{message}, url => $url };
        }
...
__PACKAGE__->register_method({
    ...
    parameters => { additionalProperties => 0,
        properties => {
            ...
            force => { description => "Always connect to server", type => 'boolean', optional => 1, default => 0 },
        },
    },
    returns => { type => 'null' },
    code => sub {
        ...
        die "Updating offline key not possible\n" if $info->{signature};
        return undef if !$param->{force} && cache_is_valid($info);
...
__PACKAGE__->register_method({
    ...
    parameters => { additionalProperties => 0,
        properties => {
            node => get_standard_option('pve-node'),
            key => { description => "Proxmox VE subscription key", type => 'string', pattern => "\\s*${subscription_pattern}\\s*", maxLength => 32 },
        },
    },
    returns => { type => 'null' },
    code => sub {
        ...
        my $new_info = { status => 'New', key => $key, checktime => time() };
        my $req_sockets = get_sockets();
        my $server_id = PVE::API2Tools::get_hwaddress();
        ...
        my $checked_info = Proxmox::RS::Subscription::check_subscription($key, $server_id, "", "Proxmox VE", $proxy);
